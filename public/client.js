    ...
    if (response.ok) {
        ...
        document.getElementById('matches').innerHTML = '<h3>Matches:</h3>' + 
            (result.matches.length ? result.matches.map(m => `
                <p>Matched with Doc ${m.id} by ${m.username} (Uploaded: ${new Date(m.timestamp).toLocaleString()}): ${m.text} (Similarity: ${m.similarity.toFixed(2)})</p>
...
async function loadAdminDashboard() {
    ...
    console.log('Admin analytics data:', data); // Debug log
    if (!response.ok) {
    ...
    document.getElementById('analytics').innerHTML = `
        ...
        ${data.uploadedFiles && data.uploadedFiles.length ? data.uploadedFiles.map(f => `
            <p>Doc ${f.id} by ${f.username} (Scanned: ${new Date(f.timestamp).toLocaleString()}): ${f.text.substring(0, 100)}${f.text.length > 100 ? '...' : ''}</p>
